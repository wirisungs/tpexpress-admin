...
const DriverTable: React.FC = () => {
  return (
    <section className="flex flex-col px-6 pt-4 pb-6 max-md:px-5">
      <Navbar>
        <div className="flex items-center mb-2 gap-4 px-6 py-6">
          <SearchInput placeholder="Tìm kiếm tài xế" />
          <RegisterButton text="Đăng ký tài xế" />
        </div>
        <p className="text-xs font-bold px-6 text-red-600 max-md:max-w-full">
          Ghi chú: Ấn và khách hàng bất kì để xem hoặc thay đổi thông tin
        </p>
        <div className="flex flex-col flex-1 px-6 py-4 mt-4 w-full text-base text-stone-500 max-md:max-w-full bg-[#F9FAFF]">
          <TableHeader />
